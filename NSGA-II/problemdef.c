...
void nsga2_optimization::test_problem (double *xreal, double *xbin,  int **gene, double *obj, double *constr, int* bin_params, double* real_params)
{
    // Variable vector: xreal[0-2] first gen, [3-5] second gen, [6-8] third gen
    // Bin parameters:
    //  1.[0] Num o DG-s
    //  2.[1] Type o DG-s
    //  3.[2] Load Flow type
    //  REMOVED 4.[4] Contains tap changer (0 - no, everything else - yes)
    int sc2DG_num = this->bin_params[0];
    int sc2DG_type = this->bin_params[1];
    int load_flow_type = this->bin_params[2];
    int hasTapChanger = this->bin_params[3];

    if (!hasTapChanger)
    {
      if (sc2DG_num == 1 && sc2DG_type == 1)
      {
        netw.setVoltageLimits(0.945, 1.05);
        Vmin = 0.945;
      }
      else if(sc2DG_num == 1 && sc2DG_type == 2)
      {
        if(load_flow_type == 1)
        {
          netw.setVoltageLimits(0.93, 1.05);
          Vmin = 0.93;
        }
        else if (load_flow_type == 2)
        {
          netw.setVoltageLimits(0.935, 1.05);
          Vmin = 0.935;
        }
      }
    }
    ...
    netw.setLoadFlowType(load_flow_type);

    obj_empty[0] = netw.getEmptyLossesSummer12(); //Losses
    obj_empty[1] = 1.0 / netw.getEmptyVSISummer12(); //VSI

    netw.removeTempElements();

    for (int i=0; i<sc2DG_num; i++)
    {
      if (sc2DG_type == 1)
        netw.setDispGenerator(round(xreal[2*i+0]), 1, xreal[2*i+1],0);
      else if (sc2DG_type == 2)
        netw.setDispGenerator(round(xreal[2*i+0]), 1, 0, xreal[2*i+1]);
      else if (sc2DG_type == 3)
        netw.setDispGenerator(round(xreal[3*i+0]), 1, xreal[3*i+1], xreal[3*i+2]);
      else
      {
        cerr<<"Error: Wrong DG type\n";
        exit(EXIT_SUCCESS);
      }
    }

    if (sc2DG_type == 1 || sc2DG_type == 2)
      netw.setTapChanger(round(xreal[2*sc2DG_num]));
    else if (sc2DG_type == 3)
      netw.setTapChanger(round(xreal[3*sc2DG_num]));

    netw.solvePowerFlow("summer", 12);



    constr[0] = netw.getMinimumVoltageMagnitude() - Vmin;
    constr[1] = Vmax - netw.getMaximumVoltageMagnitude();
    //empty_losses - netw.getActiveLosses();
    //empty_VSI - 1.0/netw.getVoltageStabilityIndexSum();

    obj[0] = netw.getActiveLosses();
    obj[1] = 1/netw.getVoltageStabilityIndexSum();
}
#endif
...
#ifdef sch1
void nsga2_optimization::test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
